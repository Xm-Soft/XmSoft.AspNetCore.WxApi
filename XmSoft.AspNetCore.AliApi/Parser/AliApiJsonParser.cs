...
namespace XmSoft.AspNetCore.AliApi.Parser
{
    ...
    /// AliApi JSON响应通用解释器。
    /// </summary>
    public class AliApiJsonParser<T> : IAliApiParser<T> where T : AliApiResponse
    {
        private static readonly Dictionary<string, Dictionary<string, AliApiAttribute>> attrs = new Dictionary<string, Dictionary<string, AliApiAttribute>>();

        #region IAliApiParser<T> Members
        public T Parse(string body,string charset = "utf-8")
        {
            T rsp = null;
        ...
        public SignItem GetSignItem(IAliApiRequest<T> request, string responseBody)
        {
        ...
        private static Dictionary<string, AliApiAttribute> GetAliApiAttributes(Type type)
        {
            Dictionary<string, AliApiAttribute> tas = null;
            ...
            {
                tas = new Dictionary<string, AliApiAttribute>();
            }

            PropertyInfo[] pis = type.GetProperties();
            foreach (PropertyInfo pi in pis)
            {
                AliApiAttribute ta = new AliApiAttribute();
        ...
        //protected static readonly DAliApiConvert AliApiJsonConvert = delegate(IAliApiReader reader, Type type)
        //{
        //    object rsp = null;
        //    Dictionary<string, AliApiAttribute> pas = GetAliApiAttributes(type);

        //    Dictionary<string, AliApiAttribute>.Enumerator em = pas.GetEnumerator();
        //    while (em.MoveNext())
        //    {
        //        KeyValuePair<string, AliApiAttribute> kvp = em.Current;
        //        AliApiAttribute ta = kvp.Value;
        ...
        //        if (ta.ListType != null)
        //        {
        //            value = reader.GetListObjects(listName, itemName, ta.ListType, AliApiJsonConvert);
        //        }
        //        else
        //        {
        ...
        //            else
        //            {
        //                value = reader.GetReferenceObject(itemName, ta.ItemType, AliApiJsonConvert);
        //            }
        //        }
        ...
        private static string GetSignSourceData(IAliApiRequest<T> request, string body)
        {
            string rootNode = request.GetApiName().Replace(".", "_") + AliApiConstants.RESPONSE_SUFFIX;
            string errorRootNode = AliApiConstants.ERROR_RESPONSE;
        ...
        private static string ParseSignSourceData(string body, string rootNode, int indexOfRootNode)
        {
            ...
            int indexOfSign = body.IndexOf("\"" + AliApiConstants.SIGN + "\"");
            if (indexOfSign < 0)
            {
                return null;
            }
        ...
        public string EncryptSourceData(IAliApiRequest<T> request, string body, string encryptType, string encryptKey, string charset)
        {
        ...
        /// <returns></returns>
        private static EncryptParseItem parseEncryptData(IAliApiRequest<T> request, string body)
        {
            string rootNode = request.GetApiName().Replace(".", "_") + AliApiConstants.RESPONSE_SUFFIX;
            string errorRootNode = AliApiConstants.ERROR_RESPONSE;
